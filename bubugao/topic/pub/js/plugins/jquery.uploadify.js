...
SWFUpload.prototype.flashReady = function() {
    ...
    if (!a) {
        this.debug("flash回调已完成，但在Flash影片无法找到。");
        return
    }
    this.cleanUp(a);
...
SWFUpload.prototype.cleanUp = function(a) {
    try {
        if (this.movieElement && typeof(a.CallFunction) === "unknown") {
            this.debug("移除 Flash 功能 （这应该只在IE中运行，并应防止内存泄漏）");
            for (var c in a) {
                try {
                    if (typeof(a[c]) === "function") {
                        a[c] = null
...
SWFUpload.Console.writeLine = function(d) {
    ...
    try {
        ...
        alert("Exception: " + c.name + " Message: " + c.message, false, 3000)
    }
};
            ...
            onDialogClose: function(filesSelected, filesQueued, queueLength) {
                ...
                if ($.inArray('onDialogClose', settings.overrideEvents) < 0) {
                    if (this.queueData.filesErrored > 0) {
                        alert(this.queueData.errorMsg, false, 3000);
                    }
                }
            ...
            onSelect: function(file) {
                ...
                for (var n in this.queueData.files) {
                    ...
                    if (queuedFile.uploaded != true && queuedFile.name == file.name) {
                        /* 文件已经存在 */
                        /* wugeng modify 2014-12-23*/
                        alert('文件 "' + file.name + '" 在队列中已经存在.\n');
                ...
                if (settings.itemTemplate == false) {
					...
					<span class="fileName" title="${fileName} (${fileSize})">${fileName} (${fileSize})</span><span class="data"></span>\
					<div class="upload-img-box"><img class="img-error jUploadImg" src="//s1.bbgstatic.com/pub/img/blank.gif"></div>\
                    ...
                    switch (errorCode) {
                        case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
                            if (settings.queueSizeLimit > errorMsg) {
                                ...
                                this.queueData.errorMsg += '\n文件个数超出限制 (' + settings.queueSizeLimit + ').';
                            }
                            break;
                ...
                if (settings.checkExisting) {
                    $.ajax({
                        ...
                        success: function(data) {
                            if (data == 1) {
                                var overwrite = confirm('文件 "' + file.name + '" 在服务器上已经存在.\n你想要覆盖存在的文件吗?');
                                if (!overwrite) {
                                    this.cancelUpload(file.id);
                                    $('#' + file.id).remove();
