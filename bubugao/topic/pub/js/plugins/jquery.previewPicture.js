...
define(function(require, exports, module) {
	...
	$.fn.previewPicture = function(options) {
		 ...
		 * @param Array
		 *            data
		 *            数据源，例如：[{'minImg':'',midImg:'',bigImg:''},{'minImg':'',midImg:'',bigImg:''}]
		 * @param String
		 *            blankImg 空图片地址,用来临时替换img标签的src属性，防止图片不存在样式难看
		 * @param String
		 *            loadingImg 懒加载图片
		 * @param Inter
		 *            minWidth 小图片的宽度
		 * @param Inter
		 *            marginRight 图片右边距的距离
		 * @param Inter
		 *            delay 预览延迟时间单位：毫秒(ms),100ms一跳，用来防止鼠标经过就加载预览大图
		 */
		var defaults = {
			data : new Array(),
			blankImg : '//s1.bbgstatic.com/pub/img/blank.gif',
			loadingImg : '../img/loading/loading.gif',
			minWidth : 62,
			marginRight : 12,
			delay : 200
		};
		...
		var IMG = {
			 ...
			 *            src 需要加载图片的地址
			 * @param Function
			 *            fnSucceed 图片加载成功后的回调函数，支持往回调函数传回刚刚加载成功的IMG对象
			 * @param Function
			 *            fnError 图片加载失败的回调函数
			 */
		...
		// 全局变量声明
		var $this = this, offset = opt.minWidth + opt.marginRight, delayCount = 0, // 延时统计
		timer, // 定时器
		...
		var Preview = {
			...
			setPosition : function(e) {
				...
				curPageY = e.pageY, _top = curPageY - dragBoxPos.top, _left = curPageX - dragBoxPos.left;
				dragBoxPos = ePrevDragBox.offset(), dragPos.top = _top - dragSize.h / 2, dragPos.left = _left - dragSize.w / 2;
				if (0 > dragPos.top) {
					dragPos.top = 0;
				}
				if (0 > dragPos.left) {
				...
				if (dragPos.top + dragSize.h > curPicMidSize.h) {
					dragPos.top = curPicMidSize.h - dragSize.h;
				}
			...
			// 设置预览图位置
			setPrevImgPosition : function(e) {
				// 如果预览图片少于显示区域则不执行放大功能
				if (curPicBigSize.w > picBigSize.w || curPicBigSize.h > picBigSize.h) {
					ePrevBigImg.css({
						top : 0,
						left : 0
					});
					e && Preview.setPosition(e);
				} else {
					ePrevBigImg.css({
						top : (picBigSize.h - curPicBigSize.h) / 2,
						left : (picBigSize.w - curPicBigSize.w) / 2
					});
					ePrevDragBox.css({
						cursor : 'pointer'
					});
