...
define(function(require, exports, module) {
	...
	CountDown.prototype = {
		defaultSetting : {
			// 获取服务器事件的url
			// http://mall.bubugao.com/api/getTime
			url : 'http://api.mall.yunhou.com/Time?callback=jQuery111109472972675323115_1429688180309&platform=js&_=1429688180310',
			// 当前时间
			currentTime : null,
			//自定义标签
			labelCtn : 'b',
		...
		init : function(argument) {
			...
			_self.getServiceTime(function( data ){
				$(_self.container).each(function(argument) {
					data && $(this).attr('data-startTime',data);
		...
		_countDown : function(_this) {
			...
			// 开始
			// io.jsonp(getTimeUrl,{},function(data){
			// 	curObj.attr('data-startTime',data);
			// });
			...
			var initTime = setInterval(_countDown, 1000);
			// 倒计时函数
			function _countDown() {
